...
class MaskDiffusion(DiscreteDiffusionMatrixBase):
    def __init__(self,
        ...
        self.device = device
        self.mask = torch.nn.functional.one_hot(torch.tensor(self.mask_id, device=device), num_classes=self.dim).unsqueeze(1).repeat(1, self.dim).float()
    ...
    def get_qt_given_q0(self,
        ...
        prob_at_time_t = q0 if t == 0 else self.noise_fn(q0, t)
...
def p_forward(
    ...
    mask_att = torch.where(x_t == 21, torch.tensor(1), torch.tensor(0)).to(x_t.device)
    mask_att = mask_att * target_mask
    output = torch.nn.functional.one_hot(x_t, 22).float()
    probs = torch.where(mask_att.bool().unsqueeze(-1), probs, output)
    logits = torch.where(mask_att.bool().unsqueeze(-1), logits, output)
...
def compute_prior_kl(x_start, diffusion, target_mask=None, word_freq_logits=None):
    ...
    q_probs = diffusion.get_qt_given_q0(q0=x_start, t=num_steps, return_logits=False, make_one_hot=True)  # get end step
...
def compute_kl_reverse_process(x_start,
    ...
    mask_att = torch.where(x_t_plus_1 == 21, torch.tensor(1), torch.tensor(0)).to(x_start.device)
    mask_att = mask_att * target_mask
    ...
    if mask_att is not None:
        kl = (kl * mask_att).sum()
        cross_entropy = (cross_entropy * mask_att).sum()
        hybrid_loss = (hybrid_loss * mask_att).sum()
    else:
        kl = kl.sum()
...
def discrete_diffusion_predict_fn(
  ...
  """
    ...
    num_steps = timesteps.clone()
    num_steps -= num_steps % step_size
    if timesteps > 0 and num_steps == 0:
        num_steps = step_size
    ...
    def sampling_step(step, state):
        ...
        sample = torch.where(target_mask == 1, sample, ori_input).to(target_mask.device)
        if show_process:
            print(tk.batch_decode(x0, clean_up_tokenization_spaces=False))

        return SamplingState(x=sample, x0=logits, t=t - step_size)

    x = diffusion.sample_stationary(ori_input.shape)
    x = torch.where(target_mask == 1, x, ori_input).to(target_mask.device)
    if context_fn is not None:
        x = context_fn(x)

    if predict_x0:
        init_state = SamplingState(x, x, torch.tensor([num_steps], device=target_mask.device))
    else:
        init_state = SamplingState(x, None, torch.tensor([num_steps], device=target_mask.device))
